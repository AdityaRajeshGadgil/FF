setwd("~/FF/FantasyFootballAnalyticsR-master")

#library(Rglpk)
#source("R Scripts/Functions/Functions.R")
#source("R Scripts/Functions/League Settings.R")

library(ffanalytics)

week = 3
my_scrape <- scrape_data(src = c("CBS","Yahoo", "ESPN", "NFL", "FFToday", "NumberFire", "FantasySharks",
...
my_projections <- subset(my_projections,avg_type=="average")
...
write.csv(my_projections,file=paste("../week",week,".csv",sep=""),quote=FALSE,row.names = FALSE)

my_projections$pred_diff <- 2*(my_projections$pos_ecr - my_projections$pos_rank)/(my_projections$pos_ecr + my_projections$pos_rank)
my_projections <- my_projections[order(my_projections$pred_diff),] 
